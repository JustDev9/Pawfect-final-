...
export default {
  ...
  methods: {
    async handleLogin() {
      ...
      try {
        const response = await axios.post('http://localhost/Test%20system/backend/login.php', {
        ...
        if (extractedData && typeof extractedData === 'object' && extractedData !== null && 'success' in extractedData) {
            if (extractedData.success === true) {
                console.log("Login API call reported success. Processing...");
                ...
                this.$toast.success(extractedData.message || 'Login successful');

                if (extractedData.user && extractedData.user.role === 'user') {
                    this.$router.push('/home');
                } else if (extractedData.user && extractedData.user.role === 'admin') {
                    this.$router.push('/admin');
                } else {
                    console.warn('User role not defined or unexpected after successful login, redirecting to home. User data:', extractedData.user);
                    this.$router.push('/home');
                }
            } else {
                console.error("Login API call reported failure (success:false):", extractedData.message || extractedData.error);
                this.errorMessage = extractedData.message || extractedData.error || 'Login failed. Please check your credentials.';
                this.$toast.error(this.errorMessage);
            }
        } else {
            console.error('Invalid response structure from server:', extractedData);
            this.errorMessage = 'Received an invalid response from the server. Please try again.';
            this.$toast.error(this.errorMessage);
        }
        ...
        if (caughtError.response && caughtError.response.data) {
            ...
            errorMsg = responseData.error || responseData.message || 'Login failed due to a server error.';
            if (responseData.redirect) {
                console.log('Redirecting due to server error response:', responseData.redirect);
            ...
            console.error('Login error: No response received.', caughtError.request);
        } else {
            errorMsg = caughtError.message || 'An unexpected error occurred.';
            console.error('Login error (not an Axios response/request error):', caughtError.message);
        }
